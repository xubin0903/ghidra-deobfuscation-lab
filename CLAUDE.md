...
- Any change to `scripts/deobfuscation/Cff*.java` or `BcfClean.java` is validated with `tools\cff-regress.ps1` (all fixtures) before it is committed; the baseline in `tools\cff-regress.expected.json` moves only with a reason written in the commit.
